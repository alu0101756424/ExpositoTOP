package top;

import java.util.Arrays;

import es.ull.esit.utilities.ExpositoUtilities;

/**
 * @file TOPTWSolution.java
 * @brief Representación de una solución para el problema TOPTW.
 * @details Mantiene la estructura de rutas mediante arreglos de predecesores y sucesores,
 *          tiempos de espera, posiciones en ruta y control de rutas creadas/vehículos disponibles.
 *          Proporciona métodos para inicializar la solución, evaluar fitness, añadir rutas y
 *          obtener información formateada de la solución.
 * @author alu0101756424
 * @version 1.0
 * @date 2025-10-31
 */
public class TOPTWSolution {
    /**
     * @brief Marca para indicar que un valor no ha sido inicializado.
     * @details Usado para inicializar arreglos y detectar elementos sin valor válido.
     */
    public static final int NO_INITIALIZED = -1;

    /** Problema TOPTW asociado a la solución. */
    private TOPTW problem;
    /** Arreglo de predecesores por nodo. */
    private int[] predecessors;
    /** Arreglo de sucesores por nodo. */
    private int[] successors;
    /** Tiempos de espera por cliente (si aplicable). */
    private double[] waitingTime;
    /** Posición del cliente dentro de su ruta. */
    private int[] positionInRoute;

    /** Identificadores de nodo que actúan como depósitos para cada ruta creada. */
    private int[] routes;
    /** Vehículos disponibles para crear nuevas rutas. */
    private int availableVehicles;
    /** Valor de la función objetivo (fitness) de la solución. */
    private double objectiveFunctionValue;

    /**
     * @brief Constructor que crea una solución vacía asociada a un problema.
     * @param problem Instancia de {@link TOPTW} con parámetros del problema.
     * @details Inicializa estructuras de tamaño adecuado y marca entradas como no inicializadas.
     */
    public TOPTWSolution(TOPTW problem) {
        this.problem = problem;
        this.availableVehicles = this.problem.getVehicles();
        this.predecessors = new int[this.problem.getPOIs()+this.problem.getVehicles()];
        this.successors = new int[this.problem.getPOIs()+this.problem.getVehicles()];
        this.waitingTime = new double[this.problem.getPOIs()];
        this.positionInRoute = new int[this.problem.getPOIs()];
        Arrays.fill(this.predecessors, TOPTWSolution.NO_INITIALIZED);
        Arrays.fill(this.successors, TOPTWSolution.NO_INITIALIZED);
        Arrays.fill(this.waitingTime, TOPTWSolution.NO_INITIALIZED);
        Arrays.fill(this.positionInRoute, TOPTWSolution.NO_INITIALIZED);
        this.routes = new int[this.problem.getVehicles()];
        this.objectiveFunctionValue = TOPTWEvaluator.NO_EVALUATED;
    }

    /**
     * @brief Inicializa la solución para comenzar la construcción de rutas.
     * @details Resetea predecesores/sucesores, marca la primera ruta con depósito y consume
     *          un vehículo inicialmente (la primera ruta creada).
     */
    public void initSolution() {
        this.predecessors = new int[this.problem.getPOIs()+this.problem.getVehicles()];
        this.successors = new int[this.problem.getPOIs()+this.problem.getVehicles()];
        Arrays.fill(this.predecessors, TOPTWSolution.NO_INITIALIZED);
        Arrays.fill(this.successors, TOPTWSolution.NO_INITIALIZED);
        this.routes = new int[this.problem.getVehicles()];
        Arrays.fill(this.routes, TOPTWSolution.NO_INITIALIZED);
        this.routes[0] = 0;
        this.predecessors[0] = 0;
        this.successors[0] = 0;
        this.availableVehicles = this.problem.getVehicles() - 1;
    }

    /**
     * @brief Indica si un nodo dado es un depósito registrado en las rutas.
     * @param c Índice del nodo a comprobar.
     * @return true si el nodo es uno de los depósitos, false en caso contrario.
     */
    public boolean isDepot(int c) {
        for(int i = 0; i < this.routes.length; i++) {
            if(c==this.routes[i]) {
                return true;
            }
        }
        return false;
    }

    /**
     * @brief Compara esta solución con otra en base a la configuración de predecesores.
     * @param otherSolution Otra instancia de {@link TOPTWSolution}.
     * @return true si los arreglos de predecesores son iguales, false en otro caso.
     */
    public boolean equals(TOPTWSolution otherSolution) {
        for (int i = 0; i < this.predecessors.length; i++) {
            if (this.predecessors[i] != otherSolution.predecessors[i]) {
                return false;
            }
        }
        return true;
    }

    /**
     * @brief Obtiene el número de vehículos todavía disponibles.
     * @return Número de vehículos disponibles para crear nuevas rutas.
     */
    public int getAvailableVehicles() {
        return this.availableVehicles;
    }

    /**
     * @brief Obtiene el número de rutas ya creadas.
     * @return Número de rutas creadas (vehículos totales menos vehículos disponibles).
     */
    public int getCreatedRoutes() {
        return this.problem.getVehicles() - this.availableVehicles;
    }

    /**
     * @brief Obtiene la distancia entre dos nodos usando la instancia del problema.
     * @param x Índice del nodo origen.
     * @param y Índice del nodo destino.
     * @return Distancia (double) entre ambos nodos.
     */
    public double getDistance(int x, int y) {
        return this.problem.getDistance(x, y);
    }

    /**
     * @brief Ajusta el número de vehículos disponibles.
     * @param availableVehicles Nuevo número de vehículos disponibles.
     */
    public void setAvailableVehicles(int availableVehicles) {
        this.availableVehicles = availableVehicles;
    }

    /**
     * @brief Obtiene el predecesor del cliente indicado.
     * @param customer Índice del cliente.
     * @return Índice del predecesor.
     */
    public int getPredecessor(int customer) {
        return this.predecessors[customer];
    }

    /**
     * @brief Devuelve el arreglo de predecesores completo.
     * @return Arreglo de enteros con los predecesores.
     */
    public int[] getPredecessors() {
        return this.predecessors;
    }

    /**
     * @brief Obtiene la instancia de problema asociada.
     * @return Instancia {@link TOPTW}.
     */
    public TOPTW getProblem() {
        return this.problem;
    }

    /**
     * @brief Obtiene el valor de la función objetivo almacenado.
     * @return Valor double del objective function.
     */
    public double getObjectiveFunctionValue() {
        return this.objectiveFunctionValue;
    }

    /**
     * @brief Obtiene la posición del cliente dentro de su ruta.
     * @param customer Índice del cliente.
     * @return Posición (int) dentro de la ruta.
     */
    public int getPositionInRoute(int customer) {
        return this.positionInRoute[customer];
    }

    /**
     * @brief Obtiene el sucesor del cliente indicado.
     * @param customer Índice del cliente.
     * @return Índice del sucesor.
     */
    public int getSuccessor(int customer) {
        return this.successors[customer];
    }

    /**
     * @brief Devuelve el arreglo de sucesores completo.
     * @return Arreglo de sucesores.
     */
    public int[] getSuccessors() {
        return this.successors;
    }

    /**
     * @brief Obtiene el índice del depósito asociado a la ruta dada.
     * @param index Índice de la ruta (0..).
     * @return Índice del nodo depósito que identifica la ruta.
     */
    public int getIndexRoute(int index) {
        return this.routes[index];
    }

    /**
     * @brief Obtiene el tiempo de espera registrado para un cliente.
     * @param customer Índice del cliente.
     * @return Tiempo de espera (double).
     */
    public double getWaitingTime(int customer) {
        return this.waitingTime[customer];
    }

    /**
     * @brief Establece el valor de la función objetivo de la solución.
     * @param objectiveFunctionValue Nuevo valor del objective function.
     */
    public void setObjectiveFunctionValue(double objectiveFunctionValue) {
        this.objectiveFunctionValue = objectiveFunctionValue;
    }

    /**
     * @brief Establece la posición en ruta de un cliente.
     * @param customer Índice del cliente.
     * @param position Posición dentro de la ruta.
     */
    public void setPositionInRoute(int customer, int position) {
        this.positionInRoute[customer] = position;
    }

    /**
     * @brief Establece el predecesor de un cliente.
     * @param customer Índice del cliente.
     * @param predecessor Índice del predecesor.
     */
    public void setPredecessor(int customer, int predecessor) {
        this.predecessors[customer] = predecessor;
    }

    /**
     * @brief Establece el sucesor de un cliente.
     * @param customer Índice del cliente.
     * @param succesor Índice del sucesor.
     */
    public void setSuccessor(int customer, int succesor) {
        this.successors[customer] = succesor;
    }

    /**
     * @brief Establece el tiempo de espera para un cliente.
     * @param customer Índice del cliente.
     * @param waitingTime Tiempo de espera (int será convertido a double).
     */
    public void setWaitingTime(int customer, int waitingTime) {
        this.waitingTime[customer] = waitingTime;
    }

    /**
     * @brief Genera una representación detallada y formateada de la solución.
     * @return Cadena con información de rutas, tiempos, feasibility y puntuación.
     * @details Usa {@link ExpositoUtilities#getFormat(String[], int[])} para alinear columnas.
     */
    public String getInfoSolution() {
        final int COLUMN_WIDTH = 15;
        String text = "\n"+"NODES: " + this.problem.getPOIs() + "\n" + "MAX TIME PER ROUTE: " + this.problem.getMaxTimePerRoute() + "\n" + "MAX NUMBER OF ROUTES: " + this.problem.getMaxRoutes() + "\n";
        String textSolution = "\n"+"SOLUTION: "+"\n";
        double costTimeSolution = 0.0, fitnessScore = 0.0;
        boolean validSolution = true;
        for(int k = 0; k < this.getCreatedRoutes(); k++) { // rutas creadas
            String[] strings = new String[]{"\n" + "ROUTE " + k };
            int[] width = new int[strings.length];
            Arrays.fill(width, COLUMN_WIDTH);
            text += ExpositoUtilities.getFormat(strings, width) + "\n";
            strings = new String[]{"CUST NO.", "X COORD.", "Y. COORD.", "READY TIME", "DUE DATE", "ARRIVE TIME", " LEAVE TIME", "SERVICE TIME"};
            width = new int[strings.length];
            Arrays.fill(width, COLUMN_WIDTH);
            text += ExpositoUtilities.getFormat(strings, width) + "\n";
            strings = new String[strings.length];
            int depot = this.getIndexRoute(k);
            int pre=-1, suc=-1;
            double costTimeRoute = 0.0, fitnessScoreRoute = 0.0;
            pre = depot;
            int index = 0;
            strings[index++] = "" + pre;
            strings[index++] = "" + this.getProblem().getX(pre);
            strings[index++] = "" + this.getProblem().getY(pre);
            strings[index++] = "" + this.getProblem().getReadyTime(pre);
            strings[index++] = "" + this.getProblem().getDueTime(pre);
            strings[index++] = "" + 0;
            strings[index++] = "" + 0;
            strings[index++] = "" + this.getProblem().getServiceTime(pre);
            text += ExpositoUtilities.getFormat(strings, width);
            text += "\n";
            do {                // recorremos la ruta
                index = 0;
                suc = this.getSuccessor(pre);
                textSolution += pre+" - ";
                strings[index++] = "" + suc;
                strings[index++] = "" + this.getProblem().getX(suc);
                strings[index++] = "" + this.getProblem().getY(suc);
                strings[index++] = "" + this.getProblem().getReadyTime(suc);
                strings[index++] = "" + this.getProblem().getDueTime(suc);
                costTimeRoute += this.getDistance(pre, suc);
                if(costTimeRoute < (this.getProblem().getDueTime(suc))) {
                    if(costTimeRoute < this.getProblem().getReadyTime(suc)) {
                        costTimeRoute = this.getProblem().getReadyTime(suc);
                    }
                    strings[index++] = "" + costTimeRoute;
                    costTimeRoute +=  this.getProblem().getServiceTime(suc);
                    strings[index++] = "" + costTimeRoute;
                    strings[index++] = "" + this.getProblem().getServiceTime(pre);
                    if(costTimeRoute > this.getProblem().getMaxTimePerRoute()) { validSolution = false; }
                    fitnessScoreRoute += this.problem.getScore(suc);
                } else { validSolution = false; }
                pre = suc;
                text += ExpositoUtilities.getFormat(strings, width);
                text += "\n";
            } while(suc != depot);
            textSolution += suc+"\n";
            costTimeSolution += costTimeRoute;
            fitnessScore += fitnessScoreRoute;
        }
        textSolution += "FEASIBLE SOLUTION: "+validSolution+"\n"+"SCORE: "+fitnessScore+"\n"+"TIME COST: "+costTimeSolution+"\n";
        return textSolution+text;
    }

    /**
     * @brief Calcula la función objetivo (fitness) sumando puntuaciones por ruta.
     * @return Valor double del fitness calculado.
     * @details Recorre cada ruta creada sumando los scores de los clientes visitados.
     */
    public double evaluateFitness() {
        double objectiveFunction = 0.0;
        double objectiveFunctionPerRoute = 0.0;
        for(int k = 0; k < this.getCreatedRoutes(); k++) {
            int depot = this.getIndexRoute(k);
            int pre=depot, suc = -1;
            do {
                suc = this.getSuccessor(pre);
                objectiveFunctionPerRoute = objectiveFunctionPerRoute + this.problem.getScore(suc);
                pre = suc;
            } while((suc != depot));
            objectiveFunction = objectiveFunction + objectiveFunctionPerRoute;
            objectiveFunctionPerRoute = 0.0;
        }
        return objectiveFunction;
    }

    /**
     * @brief Añade una nueva ruta (depósito) a la solución.
     * @return Índice del nuevo depósito creado.
     * @details Actualiza contadores, marca predecesor/sucesor del nuevo depósito y añade el
     *          nodo depósito al problema mediante {@link TOPTW#addNodeDepot()}.
     */
    public int addRoute() {
        int depot = this.problem.getPOIs();
        depot++;
        int routePos = 1;
        for(int i = 0; i < this.routes.length; i++) {
            if(this.routes[i] != -1 && this.routes[i] != 0) {
                depot = this.routes[i];
                depot++;
                routePos = i+1;
            }
        }
        this.routes[routePos] = depot;
        this.availableVehicles--;
        this.predecessors[depot] = depot;
        this.successors[depot] = depot;
        this.problem.addNodeDepot();
        return depot;
    }

    /**
     * @brief Imprime por consola la solución en formato compact y retorna su fitness.
     * @return Valor del fitness calculado.
     */
    public double printSolution() {
        for(int k = 0; k < this.getCreatedRoutes(); k++) {
            int depot = this.getIndexRoute(k);
            int pre=depot, suc = -1;
            do {
                suc = this.getSuccessor(pre);
                System.out.print(pre+" - ");
                pre = suc;
            } while((suc != depot));
            System.out.println(suc+"  ");
        }
        double fitness = this.evaluateFitness();
        System.out.println("SC="+fitness);
        return fitness;
    }

}
